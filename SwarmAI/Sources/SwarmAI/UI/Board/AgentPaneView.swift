...
struct AgentPaneView: View {
    ...
    @State private var materializationProgress: CGFloat = 0
    ...
    var body: some View {
        ...
        .scaleEffect(reduceMotion ? 1 : (1 - exitProgress * 0.1))
        ...
        .background {
            Rectangle()
                ...
                .opacity(reduceMotion ? 1 : materializationProgress)
        }
        ...
        .onAppear {
            ...
            withAnimation(.spring(response: 0.5, dampingFraction: 0.75).speed(0.6)) {
    ...
    private var contentBlur: CGFloat {
        ...
        if exitProgress > 0 { return exitProgress * 8 }
        return (1 - materializationProgress) * 8
    }
    ...
    private func sendCommand() {
        ...
        if session.isRunning {
            ...
            // If session terminated, restart session with the command
            do {
                try session.start()
                session.write(cmd + "\n")
            } catch {
