...
public enum ANSIParser {
 ...
 // MARK: - Default Palette (Material Theme Dark — used when no theme is provided)
 ...
 /// Parse raw ANSI string into a styled SwiftUI AttributedString using the default palette.
 public static func parseToAttributedString(_ text: String, defaultColor: Color = .swarmTextSecondary) -> AttributedString {
 parseToAttributedString(text, defaultColor: defaultColor, standardColors: themeStandardColors, brightColors: themeBrightColors)
 }

 /// Parse raw terminal text into multiple styled AttributedString lines using the default palette.
 public static func parseLines(_ rawText: String, defaultColor: Color = .swarmTextSecondary) -> [AttributedString] {
 parseLines(rawText, defaultColor: defaultColor, standardColors: themeStandardColors, brightColors: themeBrightColors)
 }

 /// Parse raw ANSI string into a list of styled ANSISpan tokens using the default palette.
 public static func parseToSpans(_ rawText: String) -> [ANSISpan] {
 parseToSpans(rawText, standardColors: themeStandardColors, brightColors: themeBrightColors)
 }

 // MARK: - Parse with Custom Palette (theme-aware overloads)

 /// Parse raw ANSI string into a styled SwiftUI AttributedString using the provided palette.
 public static func parseToAttributedString(
 _ text: String,
 defaultColor: Color = .swarmTextSecondary,
 standardColors: [Color],
 brightColors: [Color]
 ) -> AttributedString {
 let spans = parseToSpans(text, standardColors: standardColors, brightColors: brightColors)
 ...
 /// Parse raw terminal text into multiple styled AttributedString lines using the provided palette.
 ...
 return rawLines.map { parseToAttributedString($0, defaultColor: defaultColor, standardColors: standardColors, brightColors: brightColors) }
 }

 /// Parse raw ANSI string into a list of styled ANSISpan tokens using the provided palette.
 public static func parseToSpans(
 _ rawText: String,
 standardColors: [Color],
 brightColors: [Color]
 ) -> [ANSISpan] {
 ...
 fg = color256(codes[idx + 2], standardColors: standardColors, brightColors: brightColors)
 idx += 2
 } else if idx + 4 < codes.count && codes[idx + 1] == 2 {
 let r = max(0.0, min(1.0, Double(codes[idx + 2]) / 255.0))
 let g = max(0.0, min(1.0, Double(codes[idx + 3]) / 255.0))
 let b = max(0.0, min(1.0, Double(codes[idx + 4]) / 255.0))
 ...
 case 48:
 if idx + 2 < codes.count && codes[idx + 1] == 5 {
 bg = color256(codes[idx + 2], standardColors: standardColors, brightColors: brightColors)
 idx += 2
 } else if idx + 4 < codes.count && codes[idx + 1] == 2 {
 let r = max(0.0, min(1.0, Double(codes[idx + 2]) / 255.0))
 let g = max(0.0, min(1.0, Double(codes[idx + 3]) / 255.0))
 ...
 private static func color256(_ index: Int, standardColors: [Color], brightColors: [Color]) -> Color {
 guard index >= 0 else { return standardColors[0] }
 if index < 8 {
 ...
 let i = index - 16
 let r = max(0, min(5, (i / 36) % 6))
 let g = max(0, min(5, (i / 6) % 6))
 let b = max(0, min(5, i % 6))
