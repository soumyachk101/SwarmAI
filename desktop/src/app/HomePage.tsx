...
export default function HomePage() {
 ...
 const downloadStats = useUpdateStore((s) => s.downloadStats);

 // Background silent update check after initial launch
 useEffect(() => {
 ...
 // Keep the latest dock-toggle handler reachable from the keydown listener
 // registered once at mount, avoiding a stale closure over `toggleRight`.
 const toggleRightRef = useRef(toggleRight);
 useEffect(() => { toggleRightRef.current = toggleRight; }, [toggleRight]);

 // Same pattern for voice state setter (stable but captured once in the
 // voice-event effect below).
 const setVoiceActiveRef = useRef(setVoiceActive);
 useEffect(() => { setVoiceActiveRef.current = setVoiceActive; }, [setVoiceActive]);
 ...
 useEffect(() => {
 const onVoiceToggle = () => setVoiceActiveRef.current((v: boolean) => !v);
 const onVoiceState = (e: any) => setVoiceActiveRef.current(e.detail?.active ?? true);
 window.addEventListener("swarm:voice:toggle", onVoiceToggle);
 window.addEventListener("swarm:voice:state", onVoiceState);
 return () => {
 window.removeEventListener("swarm:voice:toggle", onVoiceToggle);
 window.removeEventListener("swarm:voice:state", onVoiceState);
 };
 }, []);
 ...
 */}
 ...
 <PanelRight size={14} />
        ...
        <OverflowMenu
          items={[
            {
              id: "updates",
              label: hasUpdate ? `Update Available (v${latestRelease?.version})` : "Check for Updates",
              hint: hasUpdate ? "New release ready" : "Check latest releases",
              icon: hasUpdate ? Sparkles : Download,
              onSelect: () => setShowUpdates(true),
            },
            {
              id: "replay-splash",
 ...
 reserveRight={
   planeFullscreen
     ? 0
     : Math.max(0, (windowControlsWidth + 20) - (dockVisible ? dockWidth : 0))
 }
 />
 ...
 <span className="text-zinc-400 text-[10px]">Local Memory Bridge</span>
        ...
        <span className="ml-auto flex shrink-0 items-center gap-2">
          {/* Live In-App Update Indicator Pill */}
          {downloadStats.status === "downloading" ? (
            <button
              onClick={() => setShowUpdates(true)}
              className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-amber-500/20 border border-amber-500/40 text-amber-300 font-mono text-[10px] font-semibold animate-pulse cursor-pointer shadow-xs"
              title={`Downloading update: ${downloadStats.percentage}% (${downloadStats.speedFormatted})`}
            >
              <RefreshCw size={11} className="animate-spin text-amber-400 shrink-0" />
              <span>Updating {downloadStats.percentage}%</span>
            </button>
          ) : downloadStats.status === "completed" ? (
            <button
              onClick={() => setShowUpdates(true)}
              className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 font-mono text-[10px] font-semibold cursor-pointer shadow-xs hover:bg-emerald-500/30 transition-all animate-pulse"
            ...
            <button
              onClick={() => setShowUpdates(true)}
              className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-amber-500/15 border border-amber-500/30 text-amber-300 hover:bg-amber-500/25 transition-all font-mono text-[10px] font-semibold cursor-pointer shadow-xs animate-pulse"
              title={`New SwarmAI version ${latestRelease?.version} available. Click to review.`}
            >
