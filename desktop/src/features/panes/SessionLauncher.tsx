...
export default function SessionLauncher({
 ...
 <div
 role="radiogroup"
 aria-label="Preset session type"
 onKeyDown={(e) => {
 const items = PRESETS;
 const currentIndex = items.findIndex((p) => p.id === selectedPreset);
 if (e.key === "ArrowRight") {
 e.preventDefault();
 const nextIndex = currentIndex < items.length - 1 ? currentIndex + 1 : 0;
 setSelectedPreset(items[nextIndex].id);
 } else if (e.key === "ArrowLeft") {
 e.preventDefault();
 const prevIndex = currentIndex > 0 ? currentIndex - 1 : items.length - 1;
 setSelectedPreset(items[prevIndex].id);
 } else if (e.key === "Home") {
 e.preventDefault();
 setSelectedPreset(items[0].id);
 } else if (e.key === "End") {
 e.preventDefault();
 setSelectedPreset(items[items.length - 1].id);
 }
 }}
 className="grid grid-cols-2 @[560px]:grid-cols-4 gap-2.5 sm:gap-3"
 >
 {PRESETS.map((p) => {
 ...
 className="grid grid-cols-2 @[520px]:grid-cols-3 @[720px]:grid-cols-4 gap-2"
 >
