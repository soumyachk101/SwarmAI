...
const STAGGER_DELAY = 30; // ms between each child animation
...
export default function GitControlModal({
 ...
 const [animState, setAnimState] = useState<"idle" | "entering" | "entered" | "exiting">("idle");

 const focusTrap = useFocusTrap({
 onEscape: onClose,
 enabled: animState === "entered",
 });
 ...
 // Animation lifecycle: entering -> entered -> (on close) exiting -> idle
 useEffect(() => {
 if (isOpen && animState === "idle") {
 setActionOutput(null);
 refreshGitState();
 setAnimState("entering");
 // Small delay lets the browser paint the enter animation frame
 const t = setTimeout(() => setAnimState("entered"), 40);
 return () => clearTimeout(t);
 }
 if (!isOpen && (animState === "entering" || animState === "entered")) {
 ...
 // Refresh git state when entering
 useEffect(() => {
 if (isOpen && animState === "entering") {
 refreshGitState();
 }
 }, [isOpen, animState, refreshGitState]);
 ...
 const isEntering = animState === "entering" || animState === "entered";

 function staggerStyle(index: number): Record<string, string> {
 const base: Record<string, string> = { opacity: "1", transform: "none" };
 if (!isEntering) return base;
 return { ...base, "--stagger-delay": `${index * STAGGER_DELAY}ms` };
 }

 const overlayClass = isEntering ? "animate-overlay-enter" : "animate-overlay-exit";
 const modalClass = isEntering ? "animate-modal-enter" : "animate-modal-exit";

 return (
 <div
 className={`fixed inset-0 z-[600] flex items-center justify-center p-4 ${overlayClass}`}
 style={{
 ...
 >
 <div
 ref={focusTrap as React.Ref<HTMLDivElement>}
 className={`w-full max-w-xl rounded-2xl border border-zinc-700/60 bg-[#12151D] shadow-2xl overflow-hidden flex flex-col ${modalClass}`}
 style={{
 boxShadow: "0 24px 64px -20px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.05)",
 }}
 onClick={(e) => e.stopPropagation()}
 >
 {/* Header */}
 <div
 className="modal-stagger-child flex items-center justify-between px-5 py-4 border-b border-zinc-800/80 bg-zinc-950/60"
 ...
 <div
 className="modal-stagger-child p-5 rounded-xl border border-zinc-700/60 bg-zinc-900/40 text-center flex flex-col items-center gap-3"
 ...
 {/* Sync Actions Bar */}
 <div
 className="modal-stagger-child grid grid-cols-2 gap-2"
 style={staggerStyle(1)}
 >
 <button
 onClick={handlePush}
 ...
 {/* Commit Staging Card */}
 <div
 className="modal-stagger-child p-4 rounded-xl border border-zinc-800 bg-zinc-900/50 flex flex-col gap-3"
 ...
 <div
 className="modal-stagger-child p-4 rounded-xl border border-zinc-800 bg-zinc-900/50 flex flex-col gap-2.5"
 ...
 {(diffStats || fullDiff) && (
 <div
 className="modal-stagger-child space-y-2 rounded-xl border border-zinc-800 bg-zinc-950/60 p-3"
 ...
 {actionOutput && (
 <div
 className={`modal-stagger-child p-3 rounded-xl border text-xs flex items-start gap-2.5 ${
