...
export default function SwarmCanvasMesh({ progress, isExiting, phase }: SwarmCanvasMeshProps) {
 ...
 const stateRef = useRef({ progress, isExiting, phase });
 stateRef.current = { progress, isExiting, phase };
 ...
 useEffect(() => {
	 ...
	 const { progress: currentProgress, isExiting: currentIsExiting, phase: currentPhase } = stateRef.current;
	 const cx = width / 2;
	 const cy = height / 2;
	 const progressRatio = Math.min(1, Math.max(0, currentProgress / 100));
	 const mouseX = mouseRef.current.x;
	 const mouseY = mouseRef.current.y;
	 const exitFade = currentIsExiting ? 0.15 : 1;
	 ...
	 const flareAlpha = Math.sin(progressRatio * Math.PI) * (currentIsExiting ? 0.05 : 0.5);
	 ...
	 if (currentPhase === "lock" || currentPhase === "resolve") {
	 const surgeAlpha = currentPhase === "resolve" ? 0.35 * exitFade : 0.2 * exitFade;
	 const surgeR = (60 + (currentPhase === "resolve" ? 80 : 0)) * dpr;
