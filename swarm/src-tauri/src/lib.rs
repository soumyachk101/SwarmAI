...
async fn spawn_terminal(
    ...
    // 1. Inherit user's login shell environment (e.g. ANTHROPIC_BASE_URL, ANTHROPIC_AUTH_TOKEN, etc.)
    for (key, value) in login_shell_environment() {
        cmd.env(key, value);
    }

    // 2. API keys and variables for the CLI agent set from Settings / Frontend
    ...
    // Ensure user home and login identity are forwarded so CLI tools (Claude Code, etc.)
    // find their global config and credential store.
...
/// Filter the raw login-shell env dump to only variables the CLI agents
/// actually need. The original implementation forwarded EVERYTHING (`env`
/// output parsed line-by-line), which let dangerous hijack variables
/// (`LD_PRELOAD`, `DYLD_INSERT_LIBRARIES`, `NODE_OPTIONS`, `BASH_ENV`,
/// `PYTHONPATH`, `PERL5OPT`, `RUBYOPT`, `GIT_SSH_COMMAND`, `GCONV_PATH`,
/// …) reach the child and potentially compromise it.
///
/// Strategy: keep the login-shell probe (API keys and agent configs live in
/// `.zshrc` / `.bash_profile` / `.profile`, not in the GUI-launched process),
/// but filter each `KEY=VALUE` pair through an allowlist of safe prefixes
/// before forwarding.
fn login_shell_environment() -> &'static HashMap<String, String> {
	static ENV_MAP: OnceLock<HashMap<String, String>> = OnceLock::new();
	ENV_MAP.get_or_init(|| {
		let mut map = HashMap::new();
		#[cfg(not(windows))]
		{
			// Safe variable prefixes — covers API keys, agent config, and locale.
			// PATH is handled separately by augmented_path_env().
			let allowed_prefixes: &[&str] = &[
				// AI provider API keys and base URLs
				"ANTHROPIC_", "OPENAI_", "GEMINI_", "XAI_", "GROQ_", "MISTRAL_",
				"COHERE_", "TOGETHER_", "FIREWORKS_", "PERPLEXITY_", "DEEPSEEK_",
				"MINIMAX_", "OPENROUTER_", "MOONSHOT_",
				// Agent config
				"CLAUDE_", "CODEX_", "AGENT_", "AUTH_", "API_KEY", "API_TOKEN",
				// Cloud / infra
				"AWS_", "AZURE_", "GCP_", "GOOGLE_", "VERCEL_", "NETLIFY_",
				"SUPABASE_", "FIREBASE_", "NEON_", "PLANETSCALE_",
				// General purpose tokens and secrets
				"TOKEN", "SECRET", "CREDENTIAL", "PASSWORD",
				// Locale / terminal
				"LANG", "LC_", "TERM", "COLORTERM", "TZ",
				// Shell and editor preferences agents may read
				"EDITOR", "VISUAL", "PAGER",
				// Node / runtime — but NOT NODE_OPTIONS (injection vector)
				"NODE_ENV", "NODE_EXTRA_CA_CERTS",
				// Git (safe config, not GIT_SSH_COMMAND)
			...
			match rx.recv_timeout(std::time::Duration::from_secs(3)) {
				Ok(Ok(out)) if out.status.success() => {
					...
					for line in raw.lines() {
						if let Some((k, v)) = line.split_once('=') {
							if k.is_empty() || k == "_" { continue; }
							let key_upper = k.to_uppercase();
							if allowed_prefixes.iter().any(|p| {
								key_upper.starts_with(p) || key_upper == *p
							}) {
				...
				Ok(Ok(_)) => {
					println!("[Rust] login shell env probe exited non-zero");
				}
				Ok(Err(e)) => {
					println!("[Rust] login shell env probe failed: {e:?}");
				}
				Err(_) => {
					println!("[Rust] login shell env probe timed out after 3s");
				}
			}
		}
		map
...
#[tauri::command]
async fn read_claude_credentials() -> Result<ClaudeCredentialsResponse, String> {
 use std::process::Command;

 // First, try the legacy ~/.claude/.credentials.json file.
 ...
 let token_value = serde_json::from_str::<serde_json::Value>(&raw)
 .ok()
 .and_then(|v| {
 let t = v.get("access_token")
 .or_else(|| v.get("accessToken"))
 .or_else(|| v.get("api_key"));
 t.and_then(|tv| tv.as_str().map(|s| s.to_string()))
 });
 if token_value.is_some() {
 let expires_at = serde_json::from_str::<serde_json::Value>(&raw)
 .ok()
 .and_then(|v| v.get("expires_at")
 .and_then(|e| e.as_str().map(|s| s.to_string())));
 return Ok(ClaudeCredentialsResponse { has_token: true, token: token_value, expires_at });
 }
 }

 // v2.1+ stores credentials in the macOS Keychain under service "Claude Code-credentials"
 // account "soumyachakraborty". The keychain item holds an mcpOAuth blob; extract the
 // access token from it so the frontend can forward it as ANTHROPIC_API_KEY.
 #[cfg(target_os = "macos")]
 {
 let output = Command::new("security")
 .args(&["find-generic-password", "-s", "Claude Code-credentials",
 "-a", "soumyachakraborty", "-g"])
 .output()
 .map_err(|e| e.to_string())?;
 if output.status.success() {
 let raw = String::from_utf8_lossy(&output.stdout).to_string();
 // The password field format is: password: "...."
 let password_part = raw.split("password: \"")
 .nth(1)
 .and_then(|s| s.split('\"').next());
 if let Some(pwd) = password_part {
 // The keychain stores a JSON blob like {"mcpOAuth":{...,"accessToken":"..."}}
 if let Ok(blob) = serde_json::from_str::<serde_json::Value>(pwd) {
 if let Some(access_token) = blob.get("mcpOAuth")
 .and_then(|o| o.as_object())
 .and_then(|mappings| {
 // mcpOAuth is a map keyed by "plugin:service:id"
 // Find the first entry with a non-empty accessToken
 mappings.values()
 .filter_map(|v| v.as_object())
 .find_map(|entry| entry.get("accessToken")
 .and_then(|t| t.as_str())
 .filter(|s| !s.is_empty())
 .map(|s| s.to_string()))
 })
 {
 return Ok(ClaudeCredentialsResponse { has_token: true, token: Some(access_token), expires_at: None });
 }
 }
 }
 }
 ...
 Ok(ClaudeCredentialsResponse { has_token: false, token: None, expires_at: None })
}

#[derive(Debug, Serialize, Deserialize)]
pub struct ClaudeCredentialsResponse {
 pub has_token: bool,
...
#[derive(Debug, Serialize, Deserialize)]
pub struct GitPushPullResponse {
 pub success: bool,
 pub stdout: String,
 pub stderr: String,
}
