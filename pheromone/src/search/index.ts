...
// Single source of truth for Pheromone hybrid retrieval.
//
// IMPORTANT sql.js compatibility notes (this is why the code looks the way it
// does — see .pheromone schema reality, not the idealised schema.ts):
// 1. The real pheromone.db is created by the Rust backend and uses an FTS5
...
export class SearchEngine {
 ...
 constructor(db: PheromoneDatabase) {
 this.db = db;
 }
 ...
 //
 // When the DB was created by the JS schema (PheromoneDatabase.migrate ->
 ...
 // Fetch 5x more candidates from SQL (ordered by embedding length DESC,
 // which correlates with information content), then filter by actual cosine
 // similarity score in JS and take the top `limit` results. This avoids a
 // full-table scan in JS on databases with >1K chunks.
 ...
 ORDER BY length(embedding) DESC
 LIMIT ?`
 );
