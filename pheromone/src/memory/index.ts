...
export class MemoryManager {
  ...
  constructor(db: PheromoneDatabase, projectPath: string) {
    this.db = db;
    this.projectPath = projectPath;
  }

  async ensureStructure(): Promise<void> {
    const pheromonePath = path.join(this.projectPath, '.pheromone');
    const dirs = [
      path.join(pheromonePath, 'memory'),
      path.join(pheromonePath, 'agents', 'sessions'),
      path.join(pheromonePath, 'agents', 'summaries'),
      path.join(pheromonePath, 'tasks'),
      path.join(pheromonePath, 'index'),
    ];
    ...
    for (const file of memoryFiles) {
      const filePath = path.join(pheromonePath, 'memory', file.name);
      try {
  ...
  async readMemoryFile(relativePath: string): Promise<MemoryFile | null> {
    const fullPath = path.join(this.projectPath, '.pheromone', relativePath);
    try {
  ...
  async listMemoryFiles(): Promise<string[]> {
    const memoryPath = path.join(this.projectPath, '.pheromone', 'memory');
