 ...
 * `.pheromone/agents/handoffs/<task>.md` files, and so the format is testable
...
export function parseHandoff(content: string, taskId: string): HandoffEntry {
 ...
 const knownKeys = new Set(['Role', 'Status', 'Worktree', 'Files touched', 'Summary', 'Blocking', 'Depends-on', 'Reviewer-notes', 'Task']);
