...
EXE_SRC=$(find "$PROJECT_ROOT/swarm/src-tauri/target/release/bundle/nsis" -name "*.exe" 2>/dev/null | head -n 1 || true)
MSI_SRC=$(find "$PROJECT_ROOT/swarm/src-tauri/target/release/bundle/msi" -name "*.msi" 2>/dev/null | head -n 1 || true)
...
APK_SRC=$(find "$PROJECT_ROOT/swarm/src-tauri/gen/android" -name "*release*.apk" -o -name "*app*.apk" 2>/dev/null | head -n 1 || true)
