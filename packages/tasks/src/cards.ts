 ...
 *
 * Tasks owns board semantics — what a new card defaults to, what moving one
 ...
 * so sortOrder stays dense and per-column.
 */
export function moveCard(
 ...
 // Use spread instead of splice to avoid mutating the source array reference —
 // React's setState does shallow comparison and will skip re-renders if the
 // array reference is unchanged.
 const next = [...target.slice(0, at), updated, ...target.slice(at)];

 // Reassemble: untouched columns, then the freshly ordered target column.
 const others = rest.filter((c) => c.column !== toColumn);
 let out = [...others, ...next.map((c, i) => ({ ...c, sortOrder: i }))];
