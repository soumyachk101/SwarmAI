...
export function nodeStatus(t: TaskCard, agentStatus?: string): NodeStatus {
 if ((t.blockingReason && (t.column === "backlog" || t.column === "todo")) || agentStatus === "error") return "failed";
...
function toNode(t: TaskCard, statuses: Record<string, string>): PipelineNode {
 ...
 status: nodeStatus(t, t.agentId ? statuses[t.agentId] : undefined),
 };
}
...
export function buildPipeline(tasks: TaskCard[], statuses: Record<string, string> = {}): PipelineStage[] {
 ...
 const hasFailed = tasks.some((t) => t.blockingReason && (t.column === "backlog" || t.column === "todo") || (t.agentId && statuses[t.agentId] === "error"));
