...
const PANE_MIGRATION_VERSION = 1;
...
export function migratePanesState(
 ...
 agents?: Agent[];
 gridLayout?: GridLayout;
 __paneMigrationVersion?: number;
 };

 // Already migrated — skip the transform
 if (s.__paneMigrationVersion === PANE_MIGRATION_VERSION) {
 return { agents: s.agents ?? [], gridLayout: s.gridLayout };
 }
...
// host not ready — dock appears on next interaction
 ...
 partialize: (s) => ({ agents: s.agents, gridLayout: s.gridLayout, __paneMigrationVersion: PANE_MIGRATION_VERSION }),
