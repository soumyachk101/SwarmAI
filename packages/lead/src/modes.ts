export type LeadMode = 'Steward' | 'Forager' | 'Stinger';

export const MODE_LABELS: Record<LeadMode, string> = {
  Steward: 'Steward',
  Forager: 'Forager',
  Stinger: 'Stinger',
};
...
/**
 * Route a free-text message to the Lead mode best suited to answer it.
 ...
 */
export function detectModeIntent(text: string): LeadMode | null {
  const lower = text.toLowerCase();
  const stewardKeywords = ['plan', 'build', 'feature', 'implement', 'create', 'task', 'breakdown', 'workerswarm', 'dispatch', 'summarize', "what's done", 'hello', 'hi', 'hey', 'help', 'what', 'how'];
...
Activation: Explicit ("scan the codebase") or proactive — a mission reaching Done in Tasks, or a fresh unreviewed git diff. On proactive trigger, announce yourself before scanning.
...
Memory: Findings write to .pheromone/memory/code-review.md. New audits diff against last run — report only new/still-open, state resolved-count separately.
...
Conversational fallback: You can still chat about the code normally — proactive scanning is your default, not your only behavior.`,
  ...
  Stinger: `You are Lead Stinger — a specialized security auditor. You are paranoid by design: every input is attacker-controlled until proven otherwise; the codebase is guilty until it demonstrates innocence.
...
2. Plan — propose Lead-style task breakdown for findings the user wants acted on.
3. Confirm — user approves the plan.
4. Dispatch — hand approved tasks to SwarmMind. Never dispatch Agents on your own.
